...
(function () {
    ...
    function computeScore(hw) {
        if (!currentKw) return 0;
        const c = (hw.content || '').toLowerCase();
        const kl = currentKw;
        let count = 0, first = -1, i = 0;
        while ((i = c.indexOf(kl, i)) >= 0) { count++; if (first < 0) first = i; i += kl.length; }
        let s = 0;
        s += count * 4;                                   // 命中次数
        if (first >= 0) s += Math.max(0, 9 - first / 45); // 命中所靠前度
        if ((hw.subjectName || '').toLowerCase().includes(kl)) s += 15; // 学科名命中
        if (hw.date) {                                    // 日期新近度
    ...
    function filterByCriteria(hw, from, to) {
        if (currentKw && !(hw.content || '').toLowerCase().includes(currentKw)) return false;
    ...
    function setupFilterToggle() {
        const toggleBtn = elFilters && elFilters.querySelector('.sm-filter-toggle');
        if (!toggleBtn) return;
        toggleBtn.addEventListener('click', () => {
            elFilters.classList.toggle('open');
            toggleBtn.setAttribute('aria-expanded', elFilters.classList.contains('open'));
            syncSize();
        });
    }
